  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 bg-slate-50 border border-slate-200 rounded-2xl h-64 text-center">
      ...
      <p className="text-[10px] text-slate-500 font-medium max-w-xs mt-1.5 leading-relaxed">
        {lang === 'sw' 
          ? 'Tafadhali weka secret yenye jina GOOGLE_MAPS_ROUTES_API_KEY au GOOGLE_MAPS_API_KEY kwenye mipangilio ya AI Studio (Gear Icon ⚙️ -> Secrets) ili kuwezesha ramani halisi ya usafirishaji.'
          : 'Please add a secret named GOOGLE_MAPS_ROUTES_API_KEY or GOOGLE_MAPS_API_KEY in AI Studio Settings (Gear Icon ⚙️ -> Secrets) to enable interactive Google Maps.'}
      </p>
    </div>
  );
}
...
const cleanMoney = (value: number) => {
  const raw = String(formatCurrency(Math.max(0, Math.round(value))));
  return raw.replace(/^(TZS|TSh|TZS\s*TSh|TSh\s*TSh)\s*/i, '').trim();
};

const money = (value: number) => `TSh ${cleanMoney(value)}`;
...
export const BusinessBundleDetailPage: React.FC<BusinessBundleDetailPageProps> = ({
  ...
  const getNicheName = () => {
    ...
    if (cat.toLowerCase().includes('fashion') || cat.toLowerCase().includes('apparel')) {
      return 'Fashion & Apparel';
    }
    if (cat.toLowerCase().includes('electr') || cat.toLowerCase().includes('tech')) {
      return 'Electronics & Tech';
    }
    if (cat.toLowerCase().includes('health') || cat.toLowerCase().includes('beauty')) {
      return 'Health & Beauty';
    }
    if (cat.toLowerCase().includes('auto') || cat.toLowerCase().includes('motor')) {
      return 'Auto & Motors';
    }
    if (cat.toLowerCase().includes('super') || cat.toLowerCase().includes('food')) {
      return 'Supermarket & Food';
    }
    return 'Home & Living';
  };
        ...
        <div className="grid grid-cols-1 gap-7 xl:grid-cols-[1.1fr_1.1fr_390px] xl:items-start">
          ...
          <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xs p-4 sm:p-5">
            <div className="relative">
              ...
              <div className="absolute right-3 top-3 z-10 grid h-14 w-14 place-items-center rounded-full bg-white text-center text-sm font-black text-[#ff4c00] shadow-md border border-slate-150 ring-4 ring-orange-50/50">
                -{bundle.discountPercentage}%
              </div>

              {/* Main Stage Showcase */}
              <div className="relative group/mainstage flex min-h-[360px] sm:min-h-[420px] items-center justify-center rounded-2xl bg-[#fffbf9] p-6 border border-slate-100/50">
                {/* Left overlay arrow */}
                <button
                  type="button"
                  onClick={() => {
                ...
                {bundle.items.map((item, index) => (
                  ...
                  >
                    <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-white border border-slate-200">
                      <img src={item.images?.[0] || fallbackImage} alt="" className="h-full w-full object-cover" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[10px] font-bold text-slate-800 truncate leading-tight">{productTitle(item, lang)}</p>
                      <span className="text-[9px] font-mono text-slate-400 block mt-0.5">{money(item.price)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs sm:p-8">
              ...
              <p className="mt-3.5 text-xs sm:text-sm leading-relaxed text-slate-500">
                {lang === 'sw' 
              ...
              <div className="mt-5 rounded-2xl border border-orange-200/60 bg-[#fffcfb] p-4 flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-black text-slate-900 leading-tight">
                    {lang === 'sw' ? 'Okoa' : 'Save'} {money(Math.max(0, bundle.originalPrice - bundle.bundlePrice))}
                  </h4>
          ...
          <aside className="xl:sticky xl:top-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm relative overflow-hidden">
              ...
              <h2 className="mt-1 text-3xl font-black tracking-tight text-[#ff4c00] font-display">{money(finalTotal)}</h2>
              
              <p className="text-[11px] font-medium text-slate-400 mt-1">
                {lang === 'sw' ? 'Bei ya kila moja' : 'Price per item'} <span className="line-through font-mono">{money(originalTotal / quantity)}</span>
              </p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex gap-4 items-start sm:items-center w-full sm:w-auto min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="text-[10px] font-bold text-[#ff4c00] uppercase block mb-1">
                            {item.category || (lang === 'sw' ? 'Vifaa vya Cooking' : 'Cooking Appliances')}
                          </span>
                      ...
                      <div className="w-full sm:w-auto flex sm:flex-col items-center justify-between sm:items-end border-t border-slate-100 sm:border-0 pt-3 sm:pt-0 sm:text-right shrink-0">
                        ...
                        <p className="font-mono text-sm font-black text-slate-950 sm:mt-1">{money(item.price)}</p>
                      </div>
                    </article>
                  );
                })}
      ...
      <div className="relative bg-slate-50/50 p-4">
        <div className="flex h-24 items-center justify-center gap-1.5">
          {(images.length ? images : [fallbackImage]).map((image, index) => (
            <div key={`${image}-${index}`} className="h-14 w-14 overflow-hidden rounded-xl bg-white shadow-xxs">
              <img
                src={image}
                alt={bundle.name}
      ...
      <div className="p-4">
        ...
        <div className="mt-3.5 flex items-end justify-between gap-2 border-t border-slate-100 pt-3">
          <div>
            ...
            <p className="font-mono text-sm font-black text-slate-950">{money(bundle.bundlePrice)}</p>
          </div>
