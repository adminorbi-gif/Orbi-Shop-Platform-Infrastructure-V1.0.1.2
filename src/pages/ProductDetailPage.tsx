    ...
    <div className="fixed inset-0 z-[999999] bg-[#f8fafc] flex flex-col overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="orbi-product-detail-shell flex-1 overflow-y-auto w-full pb-36 md:pb-12" style={{ touchAction: "pan-y" }}>
        ...
        <div className="orbi-product-detail-hero rounded-[2rem] lg:rounded-[2.5rem] border border-white/70 p-3 sm:p-5 lg:p-7 grid grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(24rem,0.95fr)] gap-6 lg:gap-9 items-start">
          ...
          {/* Left Column: Images */}
          <div className="w-full flex flex-col gap-4 lg:sticky lg:top-6">
            <div 
          ...
          <div className="w-full flex flex-col pb-4">
            ...
            <div className="flex flex-col mb-5">
              ...
              {displayFamily && (
                <button 
                  ...
                  className="group flex items-center gap-3 bg-white border border-slate-200/60 shadow-sm pr-5 pl-1.5 py-1.5 rounded-full transition-all hover:bg-slate-50 hover:border-indigo-300 hover:shadow-md active:scale-[0.98] mb-4 mt-1 max-w-full text-left"
                ...
                >
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 border border-slate-200 shrink-0 flex items-center justify-center relative">
                     {product.images && product.images[imgIdx] ? (
                       <img src={product.images[imgIdx]} alt={displayFamily} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                     ) : (
            ...
            <div className="grid grid-cols-1 gap-2 mb-8 sm:grid-cols-[minmax(0,1fr)_auto]">
              ...
              <button
                onClick={() => {
                  const targetId = displaySeller.id;
                  const targetName = displaySeller.name || displaySeller.storeName || "Orbi Shop Store";
                  const targetAvatar = displaySeller.avatar || "";
                  
                  sessionStorage.setItem("targetChatUserId", targetId);
                  sessionStorage.setItem("targetChatUserName", targetName);
                  sessionStorage.setItem("targetChatUserAvatar", targetAvatar);
                  window.dispatchEvent(
                    new CustomEvent("open-chat", {
                      detail: { 
                        sellerId: targetId,
                        sellerName: targetName,
                        sellerAvatar: targetAvatar
                      }
                    })
                  );
                }}
                className="min-h-11 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold flex items-center justify-center gap-2 px-4 py-2.5 shadow-xs transition-all active:scale-[0.98] text-sm cursor-pointer"
              >
                <MessageSquare size={16} />
                <span className="text-center leading-tight">
                  {lang === "sw" ? "Mtumie Ujumbe Muuzaji" : "Message Seller"}
                </span>
              </button>
        ...
        {relatedProducts && relatedProducts.length > 0 && (
          <div className="w-full py-12 mt-4">
            <div className="max-w-7xl mx-auto">
              {/* Specialized Niche Banner - Redesigned like "What are you looking for" */}
              <div className="relative w-full rounded-[2rem] border border-slate-200/60 overflow-hidden mb-12 shadow-sm flex flex-col items-center bg-slate-50">
                {/* Dynamic Background Image */}
                <div className="absolute inset-0 z-0 opacity-15 overflow-hidden">
                  <img
                    src={product.images?.[0] || ""}
                    alt=""
                    className="w-full h-full object-cover blur-[30px] saturate-[1.5]"
                    referrerPolicy="no-referrer"
                    onError={(e) => {
                ...
                <div className="relative z-10 w-full p-4 sm:p-6 lg:p-8">
                  ...
                  <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                    <div>
                      <span className="inline-flex items-center gap-1.5 bg-indigo-50/90 text-indigo-600 border border-indigo-100/60 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-2 backdrop-blur-sm shadow-xs">
                        <Sparkles size={12} className="animate-spin-slow text-indigo-500" />
                        <span>
                          {lang === "sw" ? "Mkusanyiko wetu bora uliopendekezwa" : "Our top recommended collection"}
                        </span>
                      </span>
                  ...
                  {/* Grid Layout matching WhatAreYouLookingFor */}
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
                    
                    {/* Spotlight (left) */}
                    <div className="lg:col-span-5 bg-white/60 backdrop-blur-md rounded-2xl border border-slate-200/50 p-4 flex flex-col gap-2 overflow-hidden relative min-h-[260px] shadow-sm group cursor-pointer transition-all duration-500"
                         onClick={(e) => {
                           e.preventDefault();
                           if (onSelectProduct && relatedProducts[spotlightIndex]) {
                              onSelectProduct(relatedProducts[spotlightIndex]);
                      ...
                      <div className="absolute top-3 right-3 z-20 flex gap-1">
                        <span className="bg-amber-100/90 text-amber-800 border border-amber-200/60 text-[10px] px-2 py-1 rounded-md font-black uppercase flex items-center gap-1 backdrop-blur-xs shadow-sm">
                          <Award size={10} /> {lang === "sw" ? "Chaguo Lako" : "Personalized Pick for you"}
                        </span>
                      </div>
                      
                      {/* Big image */}
                      <div className="absolute inset-0 opacity-20 group-hover:opacity-30 transition-opacity duration-500 pointer-events-none">
                        {relatedProducts[spotlightIndex]?.images?.[0] && (
                           <motion.img 
                             key={`bg-${relatedProducts[spotlightIndex].id}`}
                             initial={{ opacity: 0 }}
                             animate={{ opacity: 1 }}
                             transition={{ duration: 0.5 }}
                             src={relatedProducts[spotlightIndex].images[0]} 
                      ...
                      <div className="flex-1 flex flex-col relative z-10 pointer-events-none">
                        <div className="flex-1 rounded-xl overflow-hidden bg-white/80 border border-slate-200/50 flex items-center justify-center p-4 relative shadow-inner group-hover:border-indigo-200 transition-colors">
                           {relatedProducts[spotlightIndex]?.images?.[0] ? (
                             <motion.img 
                               key={`img-${relatedProducts[spotlightIndex].id}`}
                               initial={{ opacity: 0, scale: 0.95 }}
                               animate={{ opacity: 1, scale: 1 }}
                               transition={{ duration: 0.5 }}
                               src={relatedProducts[spotlightIndex].images[0]} 
                               className="w-auto h-full max-h-40 object-contain group-hover:scale-105 transition-transform duration-500" 
                               alt="" 
                             />
                           ) : (
                           ...
                           <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center">
                              <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-white text-slate-900 rounded-full w-10 h-10 flex items-center justify-center shadow-lg">
                                <ArrowRight size={18} />
                              </div>
                           </div>
                        </div>
                        <div className="mt-4 bg-white/90 rounded-xl p-3 border border-slate-200/60 shadow-xs flex items-center gap-4">
                           <div className="flex-1 min-w-0">
                             <div className="text-[10px] text-indigo-600 font-extrabold uppercase tracking-wider mb-0.5">
                               {lang === "sw" ? "Unaweza pia kupenda" : "You may also like"}
                             </div>
                             <p className="text-sm font-black text-slate-900 truncate">
                                {relatedProducts[spotlightIndex]?.name || "Product"}
                             </p>
                           </div>
                           <div className="shrink-0 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg text-xs font-bold border border-indigo-100 pointer-events-auto">
                              {lang === "sw" ? "Tazama" : "View"}
                           </div>
                        </div>
                      </div>
                    ...
                    <div className="lg:col-span-7 flex flex-col justify-center min-w-0">
                      <div className="mb-3 flex items-center justify-between">
                        <span className="text-xs text-slate-500 font-extrabold tracking-widest uppercase">
                          {lang === "sw" ? "Bidhaa Zinazohusiana" : "Related Products"}
                        </span>
                        <div className="flex items-center gap-1 text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100 shadow-xs">
                      ...
                      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-none snap-x snap-mandatory px-1">
                        {relatedProducts.slice(1, 9).map((rp) => (
                          <div
                            ...
                            className="w-36 bg-white/80 hover:bg-white border border-slate-200/60 hover:border-indigo-300 rounded-2xl p-2 shrink-0 cursor-pointer snap-start transition-all duration-300 group flex flex-col justify-between backdrop-blur-md shadow-xs hover:shadow-md hover:-translate-y-1 relative"
                          >
                            <div className="w-full aspect-square rounded-xl overflow-hidden mb-3 bg-slate-50 relative border border-slate-100 p-2 flex items-center justify-center">
                              {rp.images?.[0] ? (
                                <img src={rp.images[0]} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-300" alt="" />
                              ) : (
                            ...
                            <div>
                              <h4 className="text-xs font-black text-slate-900 group-hover:text-indigo-600 line-clamp-2 transition-colors leading-snug mb-1">
                                {rp.name}
                              </h4>
              ...
              {sortedAdsList && sortedAdsList.length > 0 && (
                <div className="mb-8 overflow-hidden rounded-2xl relative bg-emerald-50/50 border border-emerald-100/50 p-4">
                  <div className="flex items-center gap-2 mb-3 px-2">
                     <span className="text-[10px] font-black text-emerald-600 tracking-widest uppercase bg-emerald-100 px-2 py-0.5 rounded-full">
                       {lang === "sw" ? "Imedhaminiwa" : "Sponsored"}
                     </span>
                     <span className="text-xs font-bold text-slate-500">
                       {lang === "sw" ? "Matoleo Maalum" : "Special Offers"}
                     </span>
                  </div>
              ...
              <div className="orbi-product-list-grid">
                {relatedProducts.slice(0, 10).map((rp) => {
                  ...
                  const categoryPath = (rp.category || '')
                    .split('::')
                    .map((part: string) => part.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^\w-]+/g, ""))
                    .filter(Boolean)
                    .join('/');
